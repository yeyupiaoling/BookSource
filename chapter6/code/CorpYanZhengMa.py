...
class YanZhenMaUtil():
    def splitimage(self,src, dstpath):
        ...
        if not os.path.exists(path4):
            os.makedirs(path4)
        # 裁剪图片并保存
        img.crop(box1).convert('L').save(path1 + '/%s.png' % uuid.uuid1())
        img.crop(box2).convert('L').save(path2 + '/%s.png' % uuid.uuid1())
        img.crop(box3).convert('L').save(path3 + '/%s.png' % uuid.uuid1())
        img.crop(box4).convert('L').save(path4 + '/%s.png' % uuid.uuid1())
